...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'SRWITemp_XXXXX'}
...
funcVersionLocalCheck(){
    if [[ -f "${installation_path}/chrome" ]]; then
        is_existed=1
        current_version_local=$("${installation_path}/chrome" --version | awk '{print $NF}')
    fi
}
...
funcDownloadAndDecompressOperation(){
    ...
    new_installed_version=$("${installation_path}/chrome" --version | awk '{print $NF}')    # Just Installed Version In System
    [[ -f "${pack_save_path}" ]] && rm -f "${pack_save_path}"
...
Exec=installation_path/chrome %u
...
funcDesktopConfiguration(){
    if [[ -d '/usr/share/applications' ]]; then
        [[ -f "${installation_path}/product_logo_48.png" ]] && ln -sf "${installation_path}/product_logo_48.png" "${pixmaps_png_path}"
        funcDesktopFileConfiguration
    fi

    if [[ -f "${installation_path}/chrome-sandbox" ]]; then
        chown root:root "${installation_path}/chrome-sandbox"
        chmod 4755 "${installation_path}/chrome-sandbox"
    fi

    if [[ -f "${installation_path}/chrome-wrapper" ]]; then
        chmod 4755 "${installation_path}/chrome-wrapper"
    fi
