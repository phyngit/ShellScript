...
#########  0-1. Singal Setting  #########
mktemp_format=${mktemp_format:-'MTemp_XXXXX'}
...
funcVersionLocalCheck(){
    if [[ -f "${installation_path}/thunderbird" ]]; then
        is_existed=1
        current_version_local=$("${installation_path}/thunderbird" --version | awk '{print $NF}')
    fi
}
...
funcDownloadAndDecompressOperation(){
    ...
    new_installed_version=$("${installation_path}/thunderbird" --version | awk '{print $NF}')    # Just Installed Version In System
...
Exec=installation_path/thunderbird %u
...
funcDesktopConfiguration(){
    if [[ -d '/usr/share/applications' ]]; then
        [[ -f "${installation_path}/chrome/icons/default/default256.png" ]] && ln -sf "${installation_path}/chrome/icons/default/default256.png" "${pixmaps_png_path}"
        funcDesktopFileConfiguration
    fi
